...
def main():
    ...
    if not args.remote_url:
        args.remote_url = input(
            'Enter new Git remote URL for origin: '
        ).strip()
    ...
    if not author or not email:
        print(
            'Error: author name and email must be provided.',
            file=sys.stderr
        )
        sys.exit(1)
    ...
    # 1) update origin URL
    subprocess.run(
        ['git', 'remote', 'set-url', 'origin', args.remote_url],
        check=True
    )

    # 2) list all commits oldest→newest
    rev = subprocess.run(
